...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.l1 = nn.Linear(self.embed_size * n_features, hidden_size)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(self.dropout_prob)
        self.l2 = nn.Linear(hidden_size, n_classes)

        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(self.l1.weight))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(self.l1.bias))
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(self.l2.weight))
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(self.l2.bias))
    ...
    def embedding_lookup(self, w):
        ...
        ### END YOUR CODE
        x = torch.stack([self.embeddings[i] for i in w]).view(w.shape[0], -1)
        return x
